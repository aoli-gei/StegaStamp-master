...
class model_save():
    def __init__(self):
        ...
        self.model = tf.saved_model.loader.load(self.sess, [tag_constants.SERVING], "saved_models/stegastamp_pretrained")

        self.input_secret_name = self.model.signature_def[signature_constants.DEFAULT_SERVING_SIGNATURE_DEF_KEY].inputs['secret'].name
        self.input_image_name = self.model.signature_def[signature_constants.DEFAULT_SERVING_SIGNATURE_DEF_KEY].inputs['image'].name
        ...
        self.output_stegastamp_name = self.model.signature_def[signature_constants.DEFAULT_SERVING_SIGNATURE_DEF_KEY].outputs['stegastamp'].name
        self.output_residual_name = self.model.signature_def[signature_constants.DEFAULT_SERVING_SIGNATURE_DEF_KEY].outputs['residual'].name
        ...
        self.data = bytearray(self.input_s + ' ' * (7 - len(self.input_s)), 'utf-8')
        self.ecc = self.bch.encode(self.data)
        self.packet = self.data + self.ecc

        self.packet_binary = ''.join(format(x, '08b') for x in self.packet)
        self.secret = [int(x) for x in self.packet_binary]
        self.secret.extend([0, 0, 0, 0])

    def change(self):  # 给图像插入水印
        screen = ImageGrab.grab()
        img1 = np.array(ImageOps.fit(screen, (400, 400)), dtype=np.float32)
        img1 /= 255.

        feed_dict = {self.input_secret: [self.secret],
                     self.input_image: [img1]}
        ...
        # raw_img = (img1 * 255).astype(np.uint8)
        ...
        return Image.fromarray(np.array(rescaled))
...
def make_screen_img(encode_param):
    try:
        screen = md.change()
        # screen.show()
        # img_resize=np.array(ImageOps.fit(screen,(400,400)),dtype=np.float32)
        ...
        img=np.array(screen,dtype=np.float32)
        bgr_img = cv2.cvtColor(np.array(screen), cv2.COLOR_RGB2BGR)  # 颜色空间转换, cv2.COLOR_RGB2BGR 将RGB格式转换成BGR格式
        return True, cv2.imencode(".jpg", bgr_img, encode_param)[1].tostring()  # 把当前图片img按照jpg格式编码
