...
TRAIN_PATH = './data/mirflickr25k/mirflickr/'   # 训练图像来源
LOGS_Path = "./logs/"                           # 这是啥，训练记录文件好像
CHECKPOINTS_PATH = './checkpoints/'             # 检查点？
SAVED_MODELS = './saved_models'                 # 训练模型的保存位置

if not os.path.exists(CHECKPOINTS_PATH):
    os.makedirs(CHECKPOINTS_PATH)               # 如果没有这个文件夹，创建之

def get_img_batch(files_list,
                  secret_size,
                  batch_size=4,
                  size=(400,400)):              # 定义获得图片集（img_batch）的函数

    batch_cover = []
    batch_secret = []

    for i in range(batch_size):
        img_cover_path = random.choice(files_list)      # 随机选取一张图片？
        try:
            img_cover = Image.open(img_cover_path).convert("RGB")   # 以RGB的形式打开？
            img_cover = ImageOps.fit(img_cover, size)               # 返回图像裁切后的版本(400*400)
            img_cover = np.array(img_cover, dtype=np.float32) / 255.    # 将图片转化成数组，元素类型为float32，然后全部除以255？
        except:                                                         # 如果发生了异常就执行下面的代码
            img_cover = np.zeros((size[0],size[1],3), dtype=np.float32) # 生成一个全为0的矩阵(400*400*3)
        batch_cover.append(img_cover)   # 插入到batch_cover中

        secret = np.random.binomial(1, .5, secret_size) # 一个二项分布函数，返回secret_size个数值，
                                                        # 约等于做重复的伯努利实验，抛硬币，正面朝上的几率是0.5
                                                        # 最终输出一个列表（只含有0，1）,通过这样产生一个01位串作为隐藏数据来进行训练
        batch_secret.append(secret) # 隐藏数据插入到batch_secret中

    batch_cover, batch_secret = np.array(batch_cover), np.array(batch_secret)   # 转化为numpy.array形式
    return batch_cover, batch_secret    # 返回

def main():
    import argparse
    '''
    argparse python自带的命令行参数解析包，用来方便的读取命令行参数
    用parser.add_argument来增加参数，default表示如果没有指定参数的默认值
    '''
    ...
    args = parser.parse_args()  # 用此来获得解析的参数

    EXP_NAME = args.exp_name # 模型名字

    files_list = glob.glob(join(TRAIN_PATH,"**/*")) # 获取数据集的各种标签所处的位置

    config = tf.ConfigProto()   # 配置tf.Session的运算方式，比如gpu运算或者是cpu运算
    config.gpu_options.allow_growth = True  #当使用GPU时候，Tensorflow运行自动慢慢达到最大GPU的内存
    sess = tf.Session(config=config)        # 使用config的配置开始计算

    height = 400    # 长度400
    width = 400 # 宽度400

    secret_pl = tf.placeholder(shape=[None,args.secret_size],dtype=tf.float32,name="input_prep")    # 构建网络的占位，实际运行时，启动一个session，程序才会真的执行，此处列是secret_size，行不定，约等于一维向量
    image_pl = tf.placeholder(shape=[None,height,width,3],dtype=tf.float32,name="input_hide")   # 图片输入层，大小400*400*3
    M_pl = tf.placeholder(shape=[None,2,8],dtype=tf.float32,name="input_transform")             # 输入层变换，大小2*8
    global_step_tensor = tf.Variable(0, trainable=False, name='global_step')                    # 总步长？这是干啥用的呢，初始值为0
    loss_scales_pl = tf.placeholder(shape=[4],dtype=tf.float32,name="input_loss_scales")        #长度为 4 的向量，输入损失？
    l2_edge_gain_pl = tf.placeholder(shape=[1],dtype=tf.float32,name="input_edge_gain") # 不知道是啥东西
    yuv_scales_pl = tf.placeholder(shape=[3],dtype=tf.float32,name="input_yuv_scales")  # 不知道是啥东西，yuv是一种图像的编码格式

    log_decode_mod_pl = tf.placeholder(shape=[],dtype=tf.float32,name="input_log_decode_mod")

    encoder = models.StegaStampEncoder(height=height, width=width)  #这个是编码器
    decoder = models.StegaStampDecoder(secret_size=args.secret_size, height=height, width=width)   # 解码器
    discriminator = models.Discriminator()  #判别器
    ...
    while global_step < args.num_steps:         # 当小于训练循环次数的时候，就一直训练
